...
const gpu::TexturePointer Texture::getGPUTexture() const { return _textureSource->getGPUTexture(); }
...
void listSupportedImageFormats() {
    std::call_once(onceListSupportedFormatsflag, [](){
        ...
        qDebug() << "List of supported Image formats:" << formats;
    });
}
...
void ImageReader::run() {
    ...
    if (originalWidth == 0 || originalHeight == 0 || imageFormat == QImage::Format_Invalid) {
        if (filenameExtension.empty()) {
            qDebug() << "QImage failed to create from content, no file extension:" << _url;
        } else {
            qDebug() << "QImage failed to create from content" << _url;
        }
        return;
    }
