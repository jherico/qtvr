...
void OffscreenQmlSurface::updateQuick() {
    ...
    uint32_t newTexture;
    if (_renderer->_escrow.fetchSignaled(newTexture)) {
        emit textureUpdated(newTexture);

        Lock lock(_mutex);
        if (!_textureLocks.count(newTexture)) {
            _renderer->_escrow.release(newTexture);
        }
    }
}
...
void OffscreenQmlSurface::lockTexture(uint32_t texture) {
    Lock lock(_mutex);
    Q_ASSERT(!_textureLocks.count(texture) || _textureLocks[texture]);
...
void OffscreenQmlSurface::releaseTextureInternal(uint32_t texture) {
    ...
    --useCount;
    if (0 == useCount) {
        _renderer->_escrow.release(texture);
...
bool OffscreenQmlSurface::eventFilter(QObject* originalDestination, QEvent* event) {
    ...
    auto window = _renderer->_quickWindow;
    switch (event->type()) {
        case QEvent::Resize: {
